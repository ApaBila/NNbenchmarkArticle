...
=====JN?? Is the last term still f\_0 ?? ====
...
sometimes be made from the \code{DESCRIPTION} file; for others we needed
trial and error.
...
\CRANpkg{nslr} \citep{R-nlsr}. \code{nlsr} uses a variant
\citep{jn77ima} of the Levenberg-Marquardt algorithm versus the plain
Gauss-Newton of \code{nls}, modifies the relative offset convergence
...
optimx. Or nmkb -- different code?? Discussion? ====== \pkg{monmlp} has
one algorithm besides BFGS, that is, \pkg{optimx}'s Nelder-Mead.
\pkg{validann} provides the same algorithm but from \code{optim}.
\pkg{validann}'s implementation is slower, as before, but ranks slightly
...
call \code{optim} in \pkg{validann}. From Table \ref{tab:RMSEscore} it
may appear that \pkg{validann}'s implementation of the Conjugate
...
second-order algorithms, users of \pkg{h2o} should be wary of the trade
...
rather slow (highest ranking algorithm for speed is \code{RMSprop} at
