...
only one layer \textcolor{blue}{with 3 neurons}. \\
Thus, despite having a ranking of 11 in minimum RMSE in the final run,
beating some of the second-order algorithms, users of \pkg{h2o} should
...
other packages (example: ``adaptive\_rate: Specify whether to enable the
adaptive learning rate (ADADELTA). This option is enabled by default.''
in link, set to false in latest run). Some users may also wish to avoid
having to set up \textsf{Java}, which is needed for this package.
