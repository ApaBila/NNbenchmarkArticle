...
# Additionnal materials on the large dataset `bWoodN1`


```{r, echo=FALSE}
#custom format function
formatResultRable <- function(Table)
{
  colnames(Table) <- c("Package::algorithm", "RMSE", "MSE", "MAE", "WAE", "time")

  Table$Package <- sapply(strsplit(Table[, "Package::algorithm"], "::"), head, n=1)
  
  Table$Algorithm <- sapply(strsplit(Table[, "Package::algorithm"], "::"), function(x) x[2])
  Table <- Table[, c("Package", "Algorithm", "RMSE", "MSE", "MAE", "WAE", "time")]
  rownames(Table) <- 1:NROW(Table)
  Table
}
...
```{r, echo=FALSE}
resPerDataSet <- read.csv("tables-extra/bWoodN1-results-nontop5.csv")
colnames(resPerDataSet) <- c("Package::algorithm", "RMSE", "MSE", "MAE", "WAE", "time")
resPerDataSet[, "Package::algorithm"] <- substr(resPerDataSet[, "Package::algorithm"], 9, nchar(resPerDataSet[, "Package::algorithm"]))
resPerDataSet <- formatResultRable(resPerDataSet)

kableExtra::kable(resPerDataSet, format = "latex", booktabs = TRUE, 
                  centering = TRUE, align="lccccc",
                  caption="Result for bWoodN1") %>%
  column_spec(1, bold = TRUE) %>%
  kable_styling(font_size=8, latex_options =c("striped","hold_position")) %>%
  footnote(general="Time in seconds", footnote_as_chunk=TRUE)
```
