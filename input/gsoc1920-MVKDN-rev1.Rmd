...
<!-- JN??: the output has a link -- should this display in the pdf rather than be a link? => TODO CD option of Rmd -->
...
<!-- JN??: have we got the right names in 2. and 3.?? TODO CD : double check -->
...
<!-- JN?? I would like to revisit this paragraph, as I find the expression clumsy. CD: this sentence should be splitted. B: Edited -->


<!-- JN: What difference are we talking about? Discussion? B: Edited according to response below. Also see edits to 916 
B: minimum RMSE in above paragraph, other metrics as well  JN: OK -->
...
<!-- TODO JN??: Do we need to mention that optim::SANN has NO CONVERGENCE TEST. It ALWAYS runs 10000 cycles, -->
...
<!-- JN??!: What do we mean by "same numbers of neurons? Discussion?? B: Edited according to response below. See also further edits regarding 1077 -->
...
<!-- Ref 2 - Comment 14 --> <!-- B??: Christophe, is this referring to the right comment? -->
\textcolor{blue}{of 5 neurons}. <!-- previously: with fewer neurons and only one layer with 3 neurons -->
<!-- B??!: if we're still talking about mFriedman it should be 5 neurons. 
Suggestion: in less than a second with only one layer of 5 neurons. B: Edited - used my suggestion of fixing the wrong data -->  
...
<!-- (example: "adaptive_rate: Specify whether to enable the adaptive learning rate (ADADELTA). This option is enabled by default." in link, set to false in latest run).  JN??! removed-->
Some users may also wish to avoid having to set up \soft{Java}, which is needed for this package.


<!-- JN??!2: We need to reword the following2 paras. Discussion??! B: Not sure how you would prefer to approach the subject of tensorflow's technical difficulties which led to it not being tested-->
...
<!-- I think new wording is acceptable. Others? -->
...
#tuning priority 2. B??!: reworded based on above explanation. What do you think?
-->
Instead users must input 2 values into 2 separate arguments (beta1 and beta2) that will then determine which algorithm is used. <!-- B: This might lead to confusion for new users? --> <!-- previously: It would be easier to use the algorithms in this package if they did not rely on the beta parameters and instead had an argument of their own. -->
<!-- JN: reworded -->
...
first-order algorithm back-propagation, in comparison to RSNNS's Std_Backpropagation or neuralnet's backprop, ranking at 18 for minimum RMSE. <!--  previously: This is one of the better performing implementations of the first-order algorithm back-propagation, ranking at 18 for minimum RMSE. -->
<!-- Ref 3 - Comment 1 -->
