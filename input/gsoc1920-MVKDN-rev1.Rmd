...
<!-- Ref 2 - Comment 14 -->
<!-- removed colour from Before 2010 JN -->
...
<!-- reword from -->
<!-- For regression and classification, the term multilayer perceptron is used interchangeably. -->
For regression and classification, it is used interchangeably with the term multilayer perceptron. The term 
<!-- Ref 2 - Comment 13 -->
...
<!-- JN??: I don't find the wording of the paragraph appealing. Can we discuss? -->
<!-- illustrate for exemplify -->
...
perceptron. 
<!-- JN??: I find this clumsy. Can we discuss? -->
As the "layer" term suggests - some terms come 
...
<!-- JN??: do we have a good reason for this?  -->
...
<!-- JN??: "didactic" rather than "educative". Is that better?  -->
...
<!-- ========= JN: Copy edit so far to here 2021-10-17 ============== -->

<!-- JN: removed extra words in next line -->
...
<!-- JN: an -> the; algorithm -> algorithms -->
...
<!-- JN: Namely -> These are; added "and were" -->
...
<!-- JN: parameter number -> number of parameters -->
...
<!-- JN: a -> the -->
Precisely, we consider the generation of Gaussian random variates $Y_i$, $i=1,\dots,n$ 
with the mean $\mu_i$ defined as

=====JN?? Is the last term still f_0 ?? ====
...
<!-- JN?? above paragraph seems quite opaque for a general reader. Can we discuss? -->
...
<!-- JN??: the output has a link -- should this display in the pdf rather than be a link? -->
...
for instance
<!-- Ref 2 - Comment 14; JN?? changed "an" to "one"; to predict -> the prediction of -->
\textcolor{blue}{one \soft{R} package allows }the prediction of the amyloidogenicity propensity of polypeptide sequences.
<!-- JN??: I'm not sure I get the meaning. Discussion? -->
<!-- Depending on the package, this could be decided by looking at the \code{DESCRIPTION} file or by trial and error. -->
<!-- B: what I originally wanted to explain here is the actual process of excluding/including packages - I'd usually go by the DESCRIPTION file/documentation first. Usually/I'd hope to find enough about the package there. Sometimes I'd have to dig deeper into the documentation, try out some functions, or dig into the inner code of the package itself-->
...
<!-- JN: Clearing -> Clear; reword about warnings -->
4. Clear up the environment for the next package.
<!-- JN??: Is this with #4 or should it be on a new line?? -->
...
<!-- JN??: have we got the right names in 2. and 3.?? -->
<!-- B: yes but perhaps Christophe may agree that the original function names are a bit confusing? tP_1data => multiple algorithms of a package are trained against 1 dataset, tP_1pkg => a package is trained on multiple datasets -->
...
<!-- JN: suggest "Scoring ease of use" -->

**3. Scoring ease of use**
...
<!-- Ref 1 - Major comment 1-->
<!-- JN??: is it standardizing a modifier of scaling?? -->
<!-- JN: added "and"  in list of packages-->
\textcolor{red}{
...
<!-- JN??: I find this very heavy reading and am not sure I follow. We should discuss so I can improve?? -->
...
<!-- JN??: "approximately 8" seems sloppy. Can we discuss? -->
<!-- B: I believe these were the packages for neural networks at the time using nnet but not for regression. Suggestion: Moreover, 8 of the neural network packages found by RWsearch outside the scope of this research were found to use nnet. 
I didn't do a deep dig into the discarded pacakges so I'm not sure if there are more. This sentence is another that is meant to prove nnet's popularity. We can remove it?-->
...
<!-- JN??: What difference are we talking about?? Discussion? 
B: minimum RMSE in above paragraph, other metrics as well -->
...
<!-- JN??: I submitted a fix for this, and nls() now can handle zero-residual problems if the user -->
<!-- provides an extra control parameter to modify the offset criterion. ?? How to mention? -->
...
Only 2 packages include both second-order algorithms and a lower-order algorithm, that is, \CRANpkg{monmlp} and \pkg{validann}. 
 ...
 ==== JN??: Nelder-Mead called from optimx? It is called via optim from optimx. Or nmkb -- different code?? Discussion? ======
 <!-- B: monmlp uses optimx, validann uses optim -->
...
<!-- JN??: Do we need to mention that optim::SANN has NO CONVERGENCE TEST. It ALWAYS runs 10000 cycles, -->
<!-- and then stops regardless of state. Worse, there is NO MEASURE OF SUCCESS, but the convergence flag -->
<!-- is always set. Discussion please?? -->

<!-- B: I'm not fond of the algorithm either, perhaps Patrice can also mention something? Patrice did a lot of initial tests on validann's algorithms -->
...
<!-- JN??: What do we mean by "same numbers of neurons? Discussion?? -->
<!-- B: we set the number of neurons according to the dataset. 3 for Dreyfus datasets, etc. What I meant here is that with the set number of neurons, h2o was already slow. But they have a high number of default neurons which is even slower with not much RMSE gain-->
...
<!-- B??: if we're still talking about mFriedman it should be 5 neurons. 
Suggestion: in less than a second with only one layer of 5 neurons-->  
...
<!-- JN??: Next line I don't know how to reword?? -->
<!-- B: it's unclear what algorithm h2o uses, other than it being 1st order. There's also many options to change algorithms that's not as explicit as other packages. Many packages with multiple algorithms have either an argument for choosing algorithms (like validann::ann's "method" argument) or externally setting up the optimizer like in keras. In h2o, changes to an argument like adaptive learning rate can mean the algorithm ADADELTA. I guess what I'm trying to point out is that for non-expert users, the lack of clarity might be misleading or confusing-->
...
<!--JN changed from: Some users also might not want to setup \soft{Java}, which is needed, although it is not as painful to setup as some external libraries. -->
...
<!-- JN??: Does this mean internal rather than user parameters?? Discussion?? -->
<!-- B: similar to h2o, no specific argument for algorithms. Well some might argue that it's better like this so users might see the difference between algorithms... like, oh this algorithm uses this parameter. This might also be because of a coding choice? I'll leave a bit of it's documentation here for you to discuss
...
<!-- JN: reworded -->
However, there are useful notes on what parameters have a higher tuning priority. 
...
<!-- JN??: meaning?? Can we discuss? -->
<!-- B: if this package is using vanilla BP, it has a better implementation of it than RSNNS's Std_Backpropagation (27 min RMSE) or neuralnet's backprop (50 min RMSE)-->
\CRANpkg{deepnet} \citep{R-deepnet}: This is one of the better performing implementations of the 
first-order algorithm back-propagation, ranking at 18 for minimum RMSE. 
<!-- Ref 3 - Comment 1 -->
