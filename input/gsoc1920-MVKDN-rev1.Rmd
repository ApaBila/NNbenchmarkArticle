...
<!-- Ref 2 - Comment 14 -->
\textcolor{blue}{Before 2010}, neural networks were often considered theoretically instead of pragmatically, partly because the algorithms used were computationally expensive. 
...
This is an issue as many slow or poor algorithms 
<!-- Ref 2 - Comment 14 -->
...
<!-- Ref 2 - Major comment 1-->
\textcolor{blue}{
Furthermore, there are also benchmarks with a specific type of application,
...
<!-- Ref 2 - Major comment 1-->
\textcolor{blue}{
None of these benchmarks deals with NN 
<!-- Ref 2 - Comment 14 -->
\textcolor{blue}{implemented in} \soft{R} packages which
...
It is rather clear from the mathematical formula above that neural networks of
...
Unfortunately, due to certain educative tools on the backpropagation and recent
...
Therefore, many \soft{R} packages have implemented such algorithms. In the case of 
...
for instance
<!-- Ref 2 - Comment 14 -->
\textcolor{blue}{an \soft{R} package allows }to predict the amyloidogenicity propensity of polypeptide sequences.
Depending on the package, this could be decided by looking at the \code{DESCRIPTION} file or by trial and error.
\textcolor{cyan}{We refer to Table 5 in Appendix D for the full list of discarded packages.}
...
It is worth mentioning many \soft{R} packages provide scaling, standardizing
...
pre-processing functions in the package.}
...
<!-- Ref 2 - Comment 14 -->
\textcolor{blue}{depicted in Fig. 3}.
...
<!-- Ref 2 - Comment 14 -->
\textcolor{blue}{Firstly, both \pkg{minpack.lm} and \pkg{nlsr}} require the full formula of the neural network including variables and parameters. 
...
From Table 2 it may appear that \pkg{validann}'s implementation of the Conjugate
...
in \soft{Python} or \soft{Java} and packages that use as much as 100,000 iterations. 
...
Some users also might not want to setup \soft{Java}, which is needed, although it is not as painful to setup as some external libraries.
...
Then the latest TensorFlow 2.2.0 became available and we hoped to be able to use the Eager Execution provided to avoid the \soft{R} Session crashing in the summer of 2020.
Unfortunately, this led to different problems with the translation between \soft{R} and
\soft{Python} so we could not use the 2019 code.
