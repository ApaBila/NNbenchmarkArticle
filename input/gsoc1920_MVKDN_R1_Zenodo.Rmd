---
title: "A 2019-2020 Review of R Neural Network Packages with NNbenchmark"
abstract: |
  In the last three decades, neural networks have evolved from an 
  academic topic to a common scientific computing tool. 
  ...
  However, to our knowledge, there is no comprehensive study which tests 
  the accuracy, the reliability, and the ease-of-use of those NN packages.
  In this paper, we test a large number of packages against a common set 
  of datasets with varying levels of complexity to benchmark and rank 
  them with statistical metrics. We restrict our evaluation to single
  hidden-layer perceptrons that perform regression. 
  We ignore packages for classification and other specialized purposes. 
  This leaves us with approximately 60 package:algorithm pairs to test. 
  The criteria used in our benchmark were: (i) accuracy, i.e. 
  the ability to find the global minima on 13 datasets, measured by the 
  Root Mean Square Error (RMSE) in a fixed number of iterations; (ii) 
  speed of the training algorithm; (iii) availability of helpful 
  utilities; (iv) quality of the documentation.
  We have given a score for each evaluation criterion to compare 
  all package:algorithm pairs in a global table. 
  Overall, 15 pairs are considered accurate and reliable and are 
  recommended for daily usage. Other packages are either less accurate, 
  slow, difficult to use, or have poor or zero documentation.
authors:
  - name: Salsabila Mahdi
    affiliation: Universitas Syiah Kuala
    address: JL. Syech Abdurrauf No.3, Aceh 23111, Indonesia 
    orcid: bila.mahdi@mhs.unsyiah.ac.id
    email: https://orcid.org/0000-0002-2559-4154
  - name: Akshaj Verma
    affiliation: Manipal Institute of Technology
    address: Manipal, Karnataka, 576104, India
    email: https://orcid.org/0000-0002-3936-0033
    orcid: akshajverma7@gmail.com
  - name: Christophe Dutang
    ...
    email: https://orcid.org/0000-0001-6732-1501
    orcid: dutang@ceremade.dauphine.fr
  - name: Patrice Kiener
    affiliation: InModelia
    address: 5 rue Malebranche, 75005 Paris, France
    email: https://orcid.org/0000-0002-0505-9920
    orcid: patrice.kiener@inmodelia.com
  - name: John C. Nash
    affiliation: Telfer School of Management, University of Ottawa
    address: 55 Laurier Avenue East, Ottawa, Ontario K1N 6N5 Canada
    email: https://orcid.org/0000-0002-2762-8039
    orcid: nashjc@uottawa.ca
keywords:
  - R packages
  - Neural networks
  - benchmark
header-includes:
- \usepackage{tikz,float,url,natbib}
- \usepackage{subcaption}
- \usepackage{colortbl,xcolor} 
- \usepackage{multirow} 
- \usepackage{longtable} 
- \newcommand{\atan}{\textrm{atan}} 
- \newcommand{\invlogit}{\textrm{invlogit}} 
- \newcommand{\soft}{\textsf} 
- \newcommand{\pkg}{\textbf}
- \newcommand{\code}{\texttt}
- \newcommand{\CRANpkg}{\textbf}
- \newcommand{\ctv}{\textit}
bibliography: gsoc1920-MVKDN-rev1.bib
biblio-style: agsm
...
The term 
<!-- Ref 2 - Comment 13 -->
"neural network" is colloquially used for different model structures and applications.
<!-- reword from -->
In both \cite{Bishop-NNPR95,ripley2007pattern} books,
...
The term 
<!-- Ref 2 - Comment 13 -->
"recurrent neural network" is mainly used in the context of autoregressive time-series while the term 
<!-- Ref 2 - Comment 13 -->
"convolutional neural network" is 
<!-- Ref 2 - Comment 14 -->
appropriate for dimension reduction and pattern recognition (images/audio/text).
...
<!-- Ref 2 - Comment 14 -->
to fit NN are available in the literature and hence 
<!-- Ref 2 - Comment 14 JN: changed bad to weak -->
weak packages are implemented on CRAN.
...
<!-- Ref 2 - Major comment 1-->
Furthermore, there are also benchmarks 
...
adaptive step. 
<!-- Ref 3 - comment 7 -->
This yields so-called first-order methods 
...
<!-- Ref 3 - comment 7 -->
This yields so-called second-order methods 
...
<!-- Ref 3 - Comment 1 -->
there is a dearth of information that would allow users to make an informed decision. 
...
A second aim of this paper is to
...
<!-- Ref 3 - Comment 3-->
We introduce a generic notation NN $a$-$b$-$c$ for a neural network
...
<!-- Ref 2 - Comment 3 -->
In practice, modelers also use piecewise differentiable functions
...
<!-- Ref 2 - Comment 10 -->
contains \code{Nx} nodes after inputs and before outputs}), 
...
centered Gaussian $\mathcal N(0, 0.1)$ distribution. 
<!-- Ref 2 - Comment 10 -->
When normalizing input/outputs, inputs $x_l$ are replaced
...
<!-- Ref 2 - Comment 14 -->
and their potential benefits over first-order methods.
...
<!-- Ref 2 - Comment 13 -->
neural network), and the \CRANpkg{qrnn} package which performs quantile regression. 
...
Using a manual grid search, we did our best to find the best
...
As in \citep{wood2011fast}, we consider the generation of 
...
Precisely, the mean is computed as 
$$
...
(with the first and last terms using $f_0$), 
...
We refer to Table \ref{tab:Discard:Pkg} in Appendix D for the full list of discarded packages.
...
<!-- Ref 3 - Comment 5 -->
containing testing functions and datasets.
...
<!-- Ref 3 - Comment 5 -->
For this paper, the training process (3.b to 3.g) is
...
\url{https://theairbend3r.github.io/NNbenchmarkWeb/index.html}.
A 
<!-- Ref 3 - Comment 5 -->
usage example \code{trainPredict\_1pkg} is given in Appendix C,
<!-- Ref 3 - Comment 5 -->
where \pkg{nnet} is tested on the fifth dataset
...
1. Utilities in \soft{R} to deal with NN
    ...
    b. scaling capabilities exist in the package = 1 star
...
over all datasets is computed by summing the ranks (of duration, minimum RMSE, median RMSE, RMSE D51) of each \code{package:algorithm} for each dataset.
...
<!-- Ref 2 - Comment 14 -->
depicted in Fig. \ref{fig:Allpkg:timemean}.
...
<!-- Ref 2 - Comment 14 -->
Non-linear least square estimation can be performed via \code{nls} from \pkg{stats},
...
when residuals are small and can handle a degenerate Jacobian at the
first iteration.


\pkg{minpack.lm} \citep{minpack.lm} offers another Marquardt approach. 
<!-- Ref 2 - Comment 14 -->
While} \pkg{nlsr} is entirely in R,
...
as good as packages using BFGS and not being slow), 
there are some reasons why users might hesitate to choose them. 
<!-- Ref 2 - Comment 14; JN: Firstly -> First, Secondly -> Second -->
First, both \pkg{minpack.lm} and \pkg{nlsr} require the full formula of the neural network including variables and parameters. 
...
<!-- Ref 2 - Comment 14 -->
Furthermore}, both packages were able to find better minima when the dataset was scaled. 
...
<!-- Ref 2 - Comment 13 -->
neural networks} for Genomic Selection in Animal Breeding, might prove useful in that perspective.
...
had also} similar issues and is even less supported.
...
has also many options available, including a convolutional layer for CNNs, more experienced users may prefer it.
...
<!-- Ref 3 - Comment 1 -->
It is} relatively fast, ranking at 23 for speed.
...
In Section 2.1 of the supplementary materials, we observe that only 
...
<!-- Ref 2 - Major comment 7-->
\url{https://theairbend3r.github.io/NNbenchmarkWeb/index.html} and
...
<!-- Ref 2 - Major comment 4-->
In the future, we plan to use a larger list of benchmark
