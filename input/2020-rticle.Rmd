...
It converged when the terminated RMSE value is relatively small^[So, we do not choose the mean absolute error (MAE) neither for overall ranking nor for convergence value as there is no consensus in the literature,  see e.g. \citep{willmott2005advantages,chai2014root}.].
...
The resulting files in the repository were mostly generated by a Windows system build 
10.0.18362.752 with a i7-8750H CPU, a Intel(R) UHD Graphics 630 and NVIDIA GeForce GTX 1060 chip, and
16 GB of RAM.
...
==> B: I think I started this list to keep track of the packages & test the citing in 2019
We can remove it now or later if your prefer
-->
...
Furthermore, the mention of Nelder-Mead in the documentation might lead users to believe that `optim`'s Nelder-Mead is used instead. 
Speed and variation between results are also not as good as other package's that use `optim`. 
...
\CRANpkg{nnet} \citep{R-nnet} differs from the two packages because it uses the C code  from `optim` (converted earlier from Fortran) instead of calling optim from R. It also only implements the BFGS algorithm. 
...
Moreover, 8 packages for neural networks, though not tested, use nnet. 
The total number of nnet dependencies found through a search through the offline database of CRAN with \CRANpkg{RWsearch} came up with 136 packages, although some might be using nnet for the multinomial log-linear models, not neural networks. 
...
Something worth noting is that nnet and validann do not have external normalization, and it is especially recommended for \CRANpkg{validann} <!-- Patrice?? -->. 
However, some of the packages depending on \CRANpkg{nnet} do have this utility and it is included in the scoring for ease of use. 
With \CRANpkg{NNbenchmark}, this is done through setting scale = TRUE in the function \code{prepare.ZZ}. 
...
### Not Recommended: 2nd order algorithms
