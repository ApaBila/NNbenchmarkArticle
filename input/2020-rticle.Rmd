...
The R Project for Statistical Computing by \citep{Rsoft20}, as any open-source platform, relies on its contributors to keep it up to date. 
...
\CRANpkg{minpack.lm} \citep{minpack.lm} offers another Marquardt approach. Where \CRANpkg{nlsr} is entirely in R,
...
That is, we consider packages \CRANpkg{nlsr}, \CRANpkg{rminer}, \CRANpkg{nnet}, \CRANpkg{validann} with algorithm BFGS and \CRANpkg{MachineShop}.
...
and \CRANpkg{validann} are a little bit behind the others.
...
Other packages in the TOP-5,  \CRANpkg{nlsr} (the best in terms of RMSE minimum) 
and \CRANpkg{validann} are efficient packages but a little bit slower in our analysis.
