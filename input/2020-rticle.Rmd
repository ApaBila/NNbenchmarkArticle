...
the CRAN's task view for machine learning, 	\url{https://CRAN.R-project.org/view=MachineLearning}, as well as the dependency for some of the other packages tested. 
...
made generic with an extensive use of the incredible \code{do.call} function from \pkg{base} package, were added:
...
An example of call to \code{trainPredict\_1pkg} is given in Appendix C.
...
The full list of score is given in Table 5 in Appendix C.
...
Figures 2 shows the minimum RMSE value per \code{package:algorithm}
...
two considered datasets. Furthermore, there is a break between the
...
1. For regression but unsuitable for the scope of our research, coded RE in Table 2.
