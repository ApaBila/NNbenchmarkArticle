...
In order to get large gradients at the first steps of the training algorithm, the following is recommended: (i) normalized inputs and outputs (Fig. 1c), (ii) odd functions like the hyperbolic tangent function or the arctangent function, (iii) small random values to initialize the parameters. A common example is values extracted from a centred Gaussian $\mathcal N(0, 0.1)$ distribution. 
...
We might have count the number of iterations, function evaluations or similar quantities, that indicate the computing effort, but this would have required a large effort in R coding in order to get comparable values between NN packages. 
...
\CRANpkg{EnsembleBase} \citep{R-EnsembleBase} maybe useful for those who wish to make model ensembles and test a grid of parameters. Although, the documentation is rather confusing.
...
Reproducibility : all our benchmark is reproducible by any user and any package maintainer.
