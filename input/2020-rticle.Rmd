...
We define an ease-of-use measure based on what we considered a user would need 
when using a neural network package for nonlinear regression, namely, utility
functions, (non-trivial) examples and sufficient documentation 
(well-written manual, vignette(s).). 
...
<!-- JN: "0 star" becomes "0 stars" -->

Hence, ease-of-use measure ranges from 0 to 4 stars which will help new users to find user-friendly NN packages.
...
Both figures show that a good overall score, say score 16 corresponding
to \CRANpkg{qrnn}, does not necessarily imply a good score on the
two datasets under consideration. Furthermore, there is a break between the
