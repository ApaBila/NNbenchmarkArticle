...
10.0.18362.752. The machine specifications are - (i) i7-8750H CPU (ii) Intel(R) UHD Graphics 630 (iii) NVIDIA GeForce GTX 1060 chip
(iv) 16 GB of RAM.
...
Furthermore the 2019 GSoC code uses all 12 datasets. <!-- final paragraph from discussion on Nov 29 --> 
<!-- CD commented 'Our package still allows access to all 12 datasets.' --> 
For convenience, all datasets are made available in \CRANpkg{NNbenchmark}.

<!-- B?? : Does the line above become repetitive because of the added lines for number of datasets?--> 
...
search fit the scope of our research. Some have no function to generate neural networks while others were not regression neural networks of the perceptron type or were only intended for very specific purposes such as...
Depending on the package, this could be decided by looking at the DESCRIPTION file or by trial and error.
...
The full list of scores is given in Table 5 in Appendix C.
...
package:algorithm given in Table 2 (last column), e.g., 8 refers to 
...
\CRANpkg{radiant.model} \citep{R-radiant.model} has an unalterable \code{maxit} of 10000 in the original package. We changed this to harmonize the parameters. 
...
2. For time series, coded TS in Table 2.
3. For classification, coded CL in Table 2. 
4. For specific purpose, coded AP in Table 2.
5. For tools to complement NN's by other packages, coded UT in Table 2.
6. Not actually neural networks and other reasons, coded XX in Table 2.
