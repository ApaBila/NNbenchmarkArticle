...
CRAN currently hosts around 80 packages (May 2020) that involve neural-network modeling; some offering more than one algorithm. 
...
\pkg{NNbenchmark}.
<!-- Finally, we provide some hints and features to guide the development of an idealized neural network package for R. JN?? Not now included? -->
...
Note that the 2020 GSoC results are available in Section 1 of the supplementary materials, \citep{suppl:material:paper}.
...
In Section 1.13 of the supplementary materials, \citep{suppl:material:paper}, the score probabilities
per \code{package:algorithm} also gives some insights how robust is the overall score. 
...
We note that 11 out of 15 of these \code{package:algorithms} use \code{optim} from \pkg{stats}. 2 of them, \CRANpkg{CaDENCE}'s BFGS \citep{R-CaDENCE} and \CRANpkg{validann}'s BFGS and L-BFGS-B \citep{R-validann}, make the call directly.
...
\pkg{nnet} \citep{R-nnet} differs from the two packages above because it uses the C code for BFGS (\code{vmmin.c})
...
included in the distributed base R and is even mentioned as the very first package in CRAN's task view for machine learning (\ctv{MachineLearning}).

Our analysis found that 6 out of 11 packages tested that use \code{optim} do so through \pkg{nnet}. 
Moreover, approximately 8 packages for neural networks, though not tested, use \pkg{nnet}.
...
The difference is probably from the change of maximum allowable weights in \pkg{rminer} to 10000 from 1000 in \pkg{nnet}, which is also probably the reason its fits are slower. 
...
It is worth noting is that \pkg{nnet} and \pkg{validann} do not have external normalization, which is especially recommended for \pkg{validann} <!-- B??: Patrice?? -->. 
...
Another package by Cannon is \pkg{qrnn} \citep{R-qrnn} which uses yet another algorithm: \code{nlm},  a "Newton-type" algorithm, from \pkg{stats}. 
...
As \pkg{keras} also has many options available, including a convolutional layer for CNNs, more experienced users may prefer it.
...
\pkg{RSNNS}  is an example of a package that directly wraps around an external library, the Stuttgart Neural Network
...
Algorithms \code{Rprop} and \code{SCG}, the best for minimum RMSE, rank at 16 and 17 respectively which is pretty good for a first-order algorithm. 
...
\pkg{deepdive} \citep{R-deepdive}: All algorithms are very good in terms of little variance between results (see its RMSEd51 nscore). 
...
first-order algorithms back-propagation, ranking at 18 for minimum RMSE. 
...
in order, are \code{slr}, \code{sag}, and traditional \code{backprop} as the worst at rank 48 out of 60 for minimum RMSE. Notes on 
...
We fit the NN packages on Simon Wood's Gaussian dataset, see \code{bWoodN1} in dataset description, which contains
...
Table 3 gives the metric performance over 20 runs of these TOP-5 five packages on \code{bWoodN1}.

We observe that the minimum RMSE (over 20 runs) is very similar for all packages, yet \pkg{rminer}
and \pkg{validann} are a little bit behind the others.
However, once again, the metrics median RMSE and RMSEd51 reveal how consistent \pkg{rminer}'s results are in comparison to other packages. This is further proved by the other metric  norms: WAE and MAE.
Regarding computation time, the best two in this class are \pkg{nnet} and \pkg{MachineShop}.
Nevertheless, these TOP-5 packages performs significantly better than other packages, see Section 2.1 of
the supplementary materials, \citep{suppl:material:paper}.
...
Figures in Section 2.2 of the supplementary materials, \citep{suppl:material:paper},
provides some insights where a package performs reasonably well 
...
\pkg{nnet}, the most recommended package of our study, ranked third in terms on
...
However, we are disappointed that many of the packages we reviewed had poor documentation, <!-- JN??: added comma -->
notably \pkg{EnsembleBase} and \pkg{keras}.
...
As the field of neural networks continues to grow, there will always be more algorithms to validate. <!-- JN? continue / continues -->
