...
Table 2 gives the RMSE and time score per package and per algorithm. The full list of score
is given in Table 5 in Appendix C.
...
colnames(Table3) <- c("Package::algorithm", "RMSE min", "RMSE median", "RMSE D51", 
                      "MAE median", "WAE median", "Time median (sec)")

kableExtra::kable(Table3, booktabs = TRUE, centering = TRUE, longtable = TRUE,
                  caption="Performance on bWoodN1") %>%
  kable_styling(font_size=7) %>%
  footnote(general="statistics taken over 20 runs.", footnote_as_chunk=TRUE)
```
