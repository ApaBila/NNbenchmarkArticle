...
This material is available at \url{https://akshajverma.com/NNbenchmarkWeb/index.html} and
\url{https://github.com/pkR-pkR/NNbenchmark}, and can be used to verify our work and, we hope, 
improve both packages and their evaluation.
Finally, we provide some hints and features to guide the development of an idealized neural network package for R.


## Introduction

The \soft{R} Project for Statistical Computing by \citep{Rsoft20}, as any opensource platform, relies on its contributors to keep it up to date. 
Neural networks (NN), inspired on the brain's own connections system, are a class of models in the growing field of machine learning for which \soft{R} has a number of tools.
...
=> B  : still checking, but have you put in a word about this yet CD (and where if yes)? Or should I?
=> CD: no. but Patrice (?) put a sentence line 184
-->
...
For all packages, the datasets were learnt as a whole and without any weighting scheme to favour one part of the datasets. 
There was no validation subset or test subset as the purpose of our study is just to verify the ability to reach good minima. 
...
\code{uDmod1} and \code{uDmod2} are univariate datasets with few observations but exhibit high nonlinear
...
created by NIST to assess nonlinear least squares regressions are of low, low and 
medium difficulty respectively.

The last univariate dataset, \code{uNeuroOne},  was also used to teach the same course 
and is now available in the proprietary software Neuro One at \url{http://www.inmodelia.com/software.html}.
...
Finally, we also consider a Simon Wood test dataset, named \code{bWoodN1}, used in 
...
\code{bWoodN1} will only be used in the second round of our analysis when the TOP5 packages
...
Whatever the source of the test datasets, there are all available in \CRANpkg{NNbenchmark}, so that maintainers or users can perform the analysis.
...
All packages that have "neural network" as a keyword in the package title or in the package description were included. 
 In May 2020, around 80 packages fall into this category.
...
the CRAN task view for machine learning, 	\url{https://CRAN.R-project.org/view=MachineLearning}, as well as a dependency for some of the other packages tested. 
...
Basically, each package was inspected 3 times. Each assessment is described in detail below.
...
Depending on the package, this could be decided as easily as looking at
the DESCRIPTION file or having to go through the process of running some code and seeing the results.


**2. Templates for testing accuracy and speed**

Benchmarking with the template that was developed in 2019 and encapsulated in the functions of 2020, keeping
...
was created as part of GSoC 2019. In GSoC 2020, 3 functions encapsulating the template were added that had been 
made generic with an extensive use of the very helpful \code{do.call} function from \pkg{base} package:
...
<!-- %% JN- I've commented this out. <Summarizing or re-reviewing the tested packages utility functions & documentation.> -->
...
we consider: a measure for the availability of appropriate utility functions; a measure for (non-trivial) examples 
and sufficient documentation (well-written manual, vignette(s)); and a measure
to rate the clarity of the \soft{R} call to fit a given neural network. Our rating is as follows.
<!-- JN: I added the idea of clarity ; CD : I'm fine with this proposal.-->
...
a good score on the two datasets under consideration. 
Furthermore, there is a break between the
TOP10 \code{package:algorithm} and others in terms of RMSE value.
Figure 6 in Appendix D displays the score probabilities per \code{package:algorithm}. 
Regarding computation time, we observe that some package:algorithms are
...
<!-- CD : I also use the same ordering for tables in Appendix -->
...
<!-- B: NOTE FOR CHOICES ON CONSISTENCY 
J: Second, not 2nd
CD: first-order not first order
-->

## Second-order algorithms

<!-- CD: should we comment the package alphabetically? should we rank package in Table 1 according to RMSE?
B : I've done a mixture of both so far.. Sometimes it makes more sense to do on or the other, but perhaps this is bad for consistency
-->
...
Our research found that 6 of the 11 packages tested that use \code{optim} do so through \CRANpkg{nnet}. 
...
algorithms. \CRANpkg{caret} \citep{R-caret}, also mentioned in the taskview, boasts 238 methods with 13 different neural
...
<!-- JN: FWIW R-core is considering a patch I've suggested for nls() to fix the zero-resid issue -->
...
<!-- CD: can we remove the above comments? -->
...
<!-- JN: removed colloquial "it's a shame" -->
**Other packages **
...
<!-- CD: this comment could be put uncommented -->
...
<!-- B?? : should tensorflow be here or in point 1? ATM, that package and some others no longer work with the latest version of TF so we judge the performance by keras in tested packages -->
...
## Further analysis of TOP5 packages
...
explanatory variable is highly nonlinear which affects deeply the fitting time.
...
\begin{figure}
    \centering
    \includegraphics[width=.8\textwidth]{img/scoreprobperpkg.png}
        \label{fig:scoreprob}
