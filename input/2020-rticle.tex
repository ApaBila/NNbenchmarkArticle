...
For the last 30 years, neural networks have evolved from an exploratory
field of research to a common tool in scientific computing. Following
this trend, CRAN hosts many packages that claim to provide neural
...
there packages that perfom better and are easier to use than other
packages? Are some better suited to beginners while others require
expert knowledge? Do these packages provide the features found in some
respected proprietary software?

This paper is, to our knowledge, the first attempt to test this quite
large number of packages against a few datasets having different levels
of complexity, and to benchmark and rank them with certain metrics. The
evaluation was conducted during summers 2019 and 2020 by the first two
authors, as students enrolled in the Google Summer of Code, mentored by
the other authors, with templates prepared by Dutang and Kiener. We have
restricted our evaluation to regression algorithms and ignore packages
for classification and specialized purposes. This still left 49 (in
2019, 60 in 2020??) package::algorithm pairs.

The criteria used in our benchmark are: (i) the accuracy, i.e.~the
ability to find the global minima on 13 datasets, measured by the Root
Mean Square Error (RMSE), (ii) the speed of the training algorithm,
(iii) the ease of use, (iv) the quality of the documentation. All
packages::algorithms were given a score for each criterion and ranked in
a global table. Overall, 12 packages incorporating 15 algorithms are
considered accurate and reliable and can be recommended. The remaining
packages -- approximately 3/4 -- should, we believe, be avoided.

To facilitate our work, the package NNbenchmark was developed, along
with templates and code to test each package::algorithm pair. The
materials are available at \url{https://github.com/pkR-pkR/} (JN??: Is
this the one to include?). Finally, we provide some hints and features
...
All the datasets we use cannot generally be modelled using a
