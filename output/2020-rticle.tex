...
Based \CRANpkg{RWsearch}'s outputs in 2019-2020, we appropriately
selected 26 packages (and discard 63 packages) for which an in-depth
analysis. Using \CRANpkg{NNbenchmark}, we ranked 60
\code{package:algorithm} and are happy to note that most of them
...
\CRANpkg{nnet}, the only recommended package of our study, ranked third
in terms on minimum RMSE, is probably the most efficient package.
Notably \CRANpkg{nnet} is used by many other packages, such as
