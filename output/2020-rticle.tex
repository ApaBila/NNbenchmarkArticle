...
some interesting non-linear feature.
...
from \pkg{base} package, were added:
...
call to \code{trainPredict\_1pkg} is given in Appendix C.
...
full list of score is given in Table 5 in Appendix C. We divide our
analysis in two groups: packages implementing second order algorithms
and packages implementing first order algorithms. Figures 2 shows the
...
a good score on the two considered datasets. Furthermore, there is a
break between the TOP10 \code{package:algorithm} and others in terms of
RMSE value.
...
\caption{\label{tab:unnamed-chunk-9}All convergence scores per package:algorithm}
\centering
\fontsize{7}{9}\selectfont
