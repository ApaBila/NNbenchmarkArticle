...
\CRANpkg{validann}::L-BFGS-B ranks mostly lower than
\CRANpkg{validann}::BFGS, despite the former method being more
...
of the motivations for \CRANpkg{nslr} \citep{R-nlsr}. nlsr uses a
...
approach. Where \CRANpkg{nlsr} is entirely in R, and also allows for
symbolic or automatic derivatives (which are not relevant to the present
study), \CRANpkg{minpack.lm} uses compiled Fortran and C code for some
...
users. The vignetts for \CRANpkg{nlsr} and earlier book
