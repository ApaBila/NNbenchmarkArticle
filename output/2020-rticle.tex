...
\CRANpkg{CaDENCE}'s documentation that optim's BFGS algorithm is used
...
\code{optim}'s Nelder-Mead is used instead. Speed and variation between
results are also not as good as other packages that use \code{optim}.
This could be because \CRANpkg{CaDENCE} is intended for probabilistic
...
\CRANpkg{validann} is clearly a package that allows a user to use all
optim's algorithms. \CRANpkg{validann}::L-BFGS-B ranks lower than
\CRANpkg{validann}::BFGS in just about everything for most runs, despite
...
L-BFGS-B algorithm. Both \CRANpkg{CaDENCE} and validann's BFGS are
...
uses the C code from \code{optim} (converted earlier from Fortran)
instead of calling optim from R. It also only implements the BFGS
algorithm. This could be what allows it to be faster. \CRANpkg{nnet} is
...
\CRANpkg{validann}::BFGS. Most likely, the different default values are
the cause of this. For instance, \CRANpkg{nnet} uses a range of initial
random weights of 0.7 while \CRANpkg{validann} uses a value of 0.5. In
spite of these results, the real reason most authors or users are likely
to choose \CRANpkg{nnet} is because it ships with base R and is even
...
through \CRANpkg{nnet}. Moreover, 8 packages for neural networks, though
not tested, use \CRANpkg{nnet}. The total number of \CRANpkg{nnet}
dependencies found through a search through the offline database of CRAN
with \CRANpkg{RWsearch} is 136 packages, although some might be using
...
packages that use \CRANpkg{nnet} for neural networks are often meta
packages with a host of other machine learning algorithms.
\CRANpkg{caret} \citep{R-caret}, also mentioned in the taskview,
...
\CRANpkg{rminer} \citep{R-rminer} is the only package dependant on nnet
...
and \CRANpkg{minpack.lm} in terms of results that are consistent and do
not vary (RMSEd51). The difference is probably from the change of
maximum allowable weights in rminer to 10000 from 1000 in
\CRANpkg{nnet}, which is also probably the reason its fits are slower.
...
Something worth noting is that \CRANpkg{nnet} and \CRANpkg{validann} do
...
NNbenchmark, this is done through setting \code{scale = TRUE} in the
function \code{prepare.ZZ}. Note that scaling might lead to complicating
...
\CRANpkg{optimx} \citep{R-optimx} incorporates in an attempt to make a
...
the moment.\\
Of all the packages, only \CRANpkg{monmlp} \citep{R-monmlp} uses
\CRANpkg{optimx} for its 2 algorithms: BFGS and Nelder-Mead. It's
...
is also the author of \CRANpkg{CaDENCE}, has once again created a
...
by Cannon is \CRANpkg{qrnn} \citep{R-qrnn} which uses yet another
algorithm: \code{nlm()}, a ``Newton-type'' algorithm, from
\CRANpkg{stats}. Although it's performance is at the bottom of second
...
for. \CRANpkg{qrnn} is designed for quantile regression neural networks,
...
stats also includes \code{nls()}, for nonlinear least squares, which
...
\CRANpkg{nslr} \citep{R-nlsr}. nlsr uses John Nash's variant of the
...
\CRANpkg{minpack.lm} \citep{minpack.lm} which uses the lmder and lmdif
...
the results of our research is \CRANpkg{snnR} \citep{R-snnR}. It's
ranked in the top 10 of the worst algorithms for minimum RMSE out of all
60 algorithms.

\hypertarget{st-order-algorithms}{%
...
13 & evclass & CL & This package provides an evidential neural network that outputs Dempster-Shafer mass functions (DESC)\\
15 & gcForest & 00 & Based on an article with "Towards an Alternative to Deep Neural Networks" in it title (DESC)\\
\addlinespace
16 & GMDH & TS & This package provides GMDH type neural network algorithms for short term forecasting on a univariate time series (DESC)\\
17 & GMDH2 & CL & This package provides GMDH type neural network algorithms for performing binary classification (DESC)\\
18 & GMDHreg & RE* & Regression using GMDH algorithms (Title in DESC). We only managed to tested the COMBI algorithm (the most basic and first in the vignette) on the multivariate datasets (it only works for multivariate datasets). It is strangely slow on the "easy" datasets, mFriedman and mRef153. The convergence is relatively not good considering the ammount of layers.\\
19 & gnn & AP & "Generative moment matching networks (GMMNs) are introduced for generating quasi-random samples from multivariate models with any underlying copula in order to compute estimates under variance reduction." (article abstract)\\
21 & hybridEnsemble & RE & Hybrid ensemble of eight different sub-ensembles (DESC)\\
\addlinespace
22 & isingLenzMC & AP & "This package provides utilities to simulate one dimensional Ising Model with Metropolis and Glauber Monte Carlo with single flip dynamics in periodic boundary conditions" (DESC)\\
23 & LilRhino & AP & This package provides binary neural networks meant for reducing data (DESC), a random forest style collection of neural networks for classification (::Random\_Brains), and code for even more purposes. Documentation is satisfyingly clear for a package for applications: a 3 layer network with an adam optimizer, with an explanation of its activation functions (::Binary\_Network). Unfortunately, the packages it chooses for neural networks are not ones we would recommend for regression. However, they may prove to be a good choice for classification in the future (???)\\
28 & neural & CL & An implementation of "a simple MLP neural network that is suitable for classification tasks" (::mlptrain)\\
29 & NeuralNetTools & UT & "Functions are available for plotting, quantifying variable importance (Garson's algorithm), conducting a sensitivity analysis (Lek's profile), and obtaining a simple list of model weights" (DESC - (Help Pages titles))\\
30 & NeuralSens & UT & A greater focus on sensitivity, with additional functions\\
\addlinespace
31 & NlinTS & TS & A non-linear version of a causality test with feed forward neural networks and a Vector Auto-Regressive Neural Network (VARNN) among some other tools for non-linear time series analysis models and causality detection (DESC)\\
32 & nnetpredint & UT & "Computing prediction intervals of neural network models (e.g.backpropagation) at certain confidence level" (DESC)\\
33 & nnfor & TS & Automatic to fully manual time series modelling with neural networks (DESC)\\
35 & nntrf & AP & Provides useful pre-processing for Machine Learning tasks through data transformation in a non-linear, supervised way with a perceptron (DESC)\\
36 & onnx & 00'00? UT? & Aims to provide an open source format for neural networks, with definitions of an extensible computation graph model, built-in operators, and standard data types (DESC)\\
\addlinespace
37 & OptimClassifier & UT & This package searches for the best amount of neurons for binary classifcation neural networks, among other types of binary classifiers (based on how Optim.NN works \& DESC)\\
38 & OSTSC & UT & A tool to solve imbalanced data for univariate time series classification with oversampling using integrated ESPO and ADASYN methods (DESC) thus improving the performance of RNN classifiers (vignette)\\
39 & passt & AP & This package provides implementation of the Probability Associator Time (PASS-T) model, a memory model based on a simple competitive artificial neural network which imitates human judgment of frequency and duration (DESC)\\
40 & pnn & CL & This package provides implementation of the Specht algorithm, 1990, for classification with four functions: learn, smooth, perf, and guess (DESC)\\
41 & polyreg & 00 & polyregression as alternative to NN, removed keyword?\\
\addlinespace
42 & predictoR & RE & A shiny interface for supervised learning with very minimal documentation. Users may be additionally confused when opening the application only to find that it's default language is Espanol, although this can be changed in the Idioma section. (DESC \& ::init\_predictor)\\
