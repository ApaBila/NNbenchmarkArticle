...
results are also not as good as other package's that use \code{optim}.
...
with \CRANpkg{RWsearch} came up with 136 packages, although some might
...
differences. The packages that use \CRANpkg{nnet} for neuralnetworks are
...
is also probably the reason it its fits are slower. \CRANpkg{traineR}
...
\CRANpkg{monmlp} uses \CRANpkg{optimx} for its 2 algorithms: BFGS and
...
Alex J. Cannon who is also the author of \CRANpkg{CaDENCE}, has once
...
Another package by Cannon is \CRANpkg{qrnn} \citep{R-qrnn} which uses
yet another algorithm: \code{nlm()}, a ``Newton-type'' algorithm from
