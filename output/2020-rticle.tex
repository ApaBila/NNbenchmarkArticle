...
\begin{Soutput}
#> 
#> Attaching package: 'NNbenchmark'
\end{Soutput}
\begin{Soutput}
#> The following objects are masked from 'package:nseht':
#> 
#>     cc, ht, ht9
\end{Soutput}
\end{Schunk}
...
made available in \CRANpkg{NNbenchmark}, so that anyone can replicate
our analysis.
...
\pkg{nlsr}, \pkg{minpack.lm}, \pkg{caret} were added because the former
...
overall score of the \code{package:algorithm} given in Table 2 (last
column), e.g., 8 refers to \pkg{validann}\code{:optim(CG)} which is a
very slow algorithm.
...
probabilities per \code{package:algorithm}. Regarding computation time,
we observe that some \code{package:algorithm} are very slow and have
poor RMSE, e.g.~41 corresponding to \pkg{AMORE}\code{:BATCHgd}. In the
...
mention of Nelder-Mead in the documentation suggests that \code{optim}'s
Nelder-Mead method is used. Speed and variation between results for
\CRANpkg{CaDENCE} are also not as good as other packages that use
\code{optim}. This could be because \CRANpkg{CaDENCE} is intended for
...
packages for neural networks, though not tested, use \CRANpkg{nnet}.

The total number of \CRANpkg{nnet} dependencies found through a search
...
\CRANpkg{caret} \citep{R-caret}, also mentioned in the task-view, boasts
...
\citep{R-rminer} is the only package dependent on \CRANpkg{nnet} that
ranks above \CRANpkg{nnet} at number 2 for minimum RMSE, and even number
...
\CRANpkg{deepdive} and \CRANpkg{minpack.lm} in terms of results that are
...
\CRANpkg{minpack.lm} \citep{minpack.lm} offers another Marquardt
approach. Where \CRANpkg{nlsr} is entirely in \textsf{R}, and also
allows for symbolic or automatic derivatives (which are not relevant to
the present study), \CRANpkg{minpack.lm} uses compiled \textsf{Fortran}
and \textsf{C} code for some important computations. Its structure is
also better adapted to use features already available in \code{nls} that
...
datasets. We suspect \CRANpkg{nlsr}'s performance on convergence would
have similarly dropped if it was possible to use \CRANpkg{nlsr} with no
...
methods in \CRANpkg{validann} from \code{optim}. From Table 2 it may
...
On the other hand, \CRANpkg{validann}'s SANN algorithm is relatively
...
\CRANpkg{h2o} should be wary of the trade off between performance and
...
consider packages \CRANpkg{nlsr}, \CRANpkg{rminer}, \CRANpkg{nnet},
\CRANpkg{validann} with algorithm BFGS and \CRANpkg{MachineShop}. We fit
...
packages, yet \CRANpkg{rminer} and \CRANpkg{validann} are a little bit
behind the others. However, once again, the metrics median RMSE and
RMSEd51 reveal how consistent \CRANpkg{rminer}'s results are in
...
class are \CRANpkg{nnet} and \CRANpkg{MachineShop}.
...
and the \(i\)th predicted response. For instance, \CRANpkg{MachineShop},
\CRANpkg{nnet}, \CRANpkg{nlsr} do not correctly capture the sinusoidal
aspect of explanatory variable \(x_5\) on the expected response, whereas
\CRANpkg{rminer}, \CRANpkg{validann} miss the increasing non-linear
trend of explanatory variable \(x_1\) on the expected response.
...
Based on \CRANpkg{RWsearch}'s outputs in 2019-2020, we selected 26
appropriate packages to analyze in-depth and discarded the other 63
packages. Using \CRANpkg{NNbenchmark}, we ranked 60
\code{package:algorithm} pairs and are happy to note that most of them
...
\CRANpkg{nnet}, the most recommended package of our study, ranked third
in terms on minimum RMSE, and is probably the most efficient package.
\CRANpkg{nnet} is notably used by many other packages, such as
\CRANpkg{MachineShop} and \CRANpkg{rminer} respectively ranked fifth and
second. \CRANpkg{MachineShop} and \CRANpkg{rminer} are also very good
challengers in our benchmark, in particular when considering a larger
dataset. Other packages in the TOP-5, \CRANpkg{nlsr} (the best in terms
of RMSE minimum) and \CRANpkg{validann} are efficient packages but a
little bit slower in our analysis.

However we are disappointed that many of the packages we reviewed had
poor documentation, notably \CRANpkg{EnsembleBase} and \CRANpkg{keras}.
...
reproducible thanks to \CRANpkg{NNbenchmark}. We hope users and package
